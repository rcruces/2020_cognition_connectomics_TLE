...
#----------------------------------------------------------------
#### Plot Lines  ####
plotLines <- function(Data,Col,x) {
  Data <- as.data.frame(Data)
  n <- length(Data[,1])
  
  # Transparense Color-Ramp
  a <- round(seq(150,250,length.out = n))
  colR <- c()
  for (i in a) { colR <- c(colR,addTrans(Col,i))}
  colR <- colR[order(Data[,1])]
  
  # Jittered X vectors
  if (length(x) > 10) {jj <- c(4,1.5,1,0.75,0.5,0.25,0.2,0.15,0.10,rep(0.05,length(x)-9))
  } else {jj <- c(4,1.5,1,0.75,0.5,0.25,0.2,0.15,0.10,0.05)}
  
  xx <- c()
  for (i in 1:length(x)) {  assign(paste0("x", i),jitter(rep(x[i],n),jj[i]))
    xx <- c(xx, paste0("x", i))}
  # Add the lines
  Lwd <- seq(2,4,length.out = n)
  for (i in 1:(length(x)-1)) { X <- cbind(get(xx[i]),get(xx[i+1]))
  for (ii in 1:n) {lines(c(X[ii,1],X[ii,2]),c(Data[ii,i],Data[ii,i+1]),col=colR[ii],lwd=Lwd[ii]) }}
  
  # Add the points
  for (i in c(1:length(x))) { points(get(xx[i]),Data[,i],col=colR,pch=19,cex=seq(0.5,1.5,length.out = n))}
}

#----------------------------------------------------------------
Lines <- function(Data,Col,x,Lwd) {
  Data <- as.data.frame(Data)
  n <- length(Data[,1])
  xx <- c()
  for (i in 1:length(x)) {  assign(paste0("x", i),rep(x[i],n))
    xx <- c(xx, paste0("x", i))}
  # Add the lines
  for (i in 1:(length(x)-1)) { X <- cbind(get(xx[i]),get(xx[i+1]))
  for (ii in 1:n) {lines(c(X[ii,1],X[ii,2]),c(Data[ii,i],Data[ii,i+1]),col=Col,lwd=Lwd) }}
}

#----------------------------------------------------------------
Points <- function(Data,Col,x,Cex) {
  Data <- as.data.frame(Data)
  n <- length(Data[,1])
  # Jittered X vectors
  if (length(x) > 10) {jj <- c(4,1.5,1,0.75,0.5,0.25,0.2,0.15,0.10,rep(0.05,length(x)-9))
  ...
  for (i in 1:length(x)) {  assign(paste0("x", i),jitter(rep(x[i],n),jj[i]))
    xx <- c(xx, paste0("x", i))}
# Add the points
  for (i in c(1:length(x))) { points(get(xx[i]),Data[,i],col=Col,pch=19,cex=Cex)}
}
...
#----------------------------------------------------------------
#### Function to color the branches of a Dendrogram #### 
...
#----------------------------------------------------------------
# FUNCTION: Cluster vector to matrix visualization
# Takes a clusterization vector as input and generates a matrix out of it
# Illustrates the visualization of a 1D clustering as 2D matrix
cluster2mtx <- function(ClusVec) { n<-length(ClusVec)
  cM<-(matrix(data=0,nrow = n,ncol = n))
  colnames(cM)<-rownames(cM)<-names(ClusVec)
  X<-ClusVec
  J<-names(X)
  for (i in names(X)) { 
    for (j in J) { 
      # Coordinates of the 
      x<-match(c(i,j),names(cM[1,]))
      if ( X[i]==X[j] ) { ij<-X[j] } else {ij<-0}
      val<-cM[x[1], x[2]]+ij
      cM[x[1], x[2]] <- cM[x[2], x[1]] <- val}
    J<-J[-match(i,J)]}
  return(cM)}


#----------------------------------------------------------------
#### I'm not very sure what this one does ####
mtx.clus <- function(clustPartition, joint) {
  X<-clustPartition
  J<-names(X)
  for (i in names(X)) { 
    for (j in J) { 
      # Coord of the ID in the joint matrix
      x<-match(c(i,j),names(joint[1,]))
      if ( X[i]==X[j] ) { ij<-X[j] } else {ij<-0}
      val<-joint[x[1], x[2]]+ij
      joint[x[1], x[2]] <- joint[x[2], x[1]] <- val}
...
#################################################################
#   THIS ONE IS A MODIFICATION FROM NbClust R-PACKAGE
#################################################################

MyNbClust<-function (data = NULL, diss = NULL, distance = "euclidean", min.nc = 2, 
                     max.nc = 15, method = NULL, index = "all", alphaBeale = 0.1, plotetc=FALSE)
  {
    x <- 0
    min_nc <- min.nc
    max_nc <- max.nc
